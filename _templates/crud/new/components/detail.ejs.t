...
type Detail<%= h.inflection.camelize(name) %>Props = {
...
] as List<ItemInfo<keyof <%= h.inflection.camelize(name) %>>>;

export const Detail<%= h.inflection.camelize(name) %> = ({ data }: Detail<%= h.inflection.camelize(name) %>Props) => {
  const [isOpen, { open, close }] = useDisclosure(false);

  const { loading, data: detail } = useRequest(
    () => <%= h.inflection.camelize(name, true) %>Service.get<%= h.inflection.camelize(name) %>(data?.id),
    {
      immediate: true,
    },
  );
